...
> **2026-09-19 量測與決定。** 正式 D1 已發布 91 期；`usage_log` 覆蓋 41 期（2026-08-25 起），
> `stories` 覆蓋 20 期（2026-08-29 起）。一期平均 7.7 個叢集，標題平均 23.5 字、最長 38 字，是句子不是標籤。
> 下面第一條推翻條件成立：歷史列拿掉主題，也不再從 `stories` 讀；篇數有 `usage_log` 紀錄才顯示。
> 頭版的主題改成記憶體裡成員最多的前兩個叢集標題，以 small 文字呈現。規範 §4、§6 已同步。
