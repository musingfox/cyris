...
**Ten numbered items are open — #9, #13, #14, #17, #28, #29, #30, #31, #32 and #33 — plus the two unnumbered rows under
...
| 33 | `doctor` says which image production runs, and how far behind it is | Nothing asked until 2026-09-15. On 2026-09-14 production was found running 2026-09-03's code with no way to date it, and printing the local sha would not have helped — `doctor` on a laptop reports the laptop. It has to be a comparison, and the online half has exactly one source: the deployment saying so itself. Cloudflare documents no way to read the image a Worker version references (no Containers resource in the API reference; a version's response carries at most a Durable Object's container name) and injects no image identity into a running container. So the `ui` role answers `/api/build` with its baked `CYRIS_GIT_SHA`, and `doctor --deployment <url>` signs in with `CYRIS_UI_TOKEN`, reads it and counts the distance to local HEAD; any request wakes that instance, so the question is answerable on demand rather than at the next cron hour. A checkout ahead of production warns rather than fails — that is every working day. First real run, 2026-09-15 against `cyris-app.<subdomain>.workers.dev`: `/login` answered 302 and the cookie carried, and `/api/build` answered **404** — production still runs the 2026-09-14 image, which predates the endpoint. That is the check working: the sign-in proves the host, so a 404 there is itself a date, and it gets its own verdict rather than the hostname hint a dead host would get. **Open on its receipt**: a green run, which needs #30's deploy path to ship a CI-built image. The runtime half — which sha the last pipeline run actually executed — waits on #32. Ticket `doctor-deployment-provenance` |
