...
| 30 | CI release image | GitHub Actions must build the amd64 image, bake `CYRIS_GIT_SHA`, push immutable and `release` registry tags, and annotate the source commit. The deploy remains separate: `wrangler deploy` still builds locally until that path changes | `release-image-build-in-ci` workflow dispatch publishes and verifies both tags. The half still open is the deploy: `wrangler deploy` builds `./Dockerfile` locally, so production runs an image with no sha and cannot be dated — `deploy-from-registry-image` owns that, and closing it also closes #33's receipt and gives #31 its pinning mechanism | `release-image-build-in-ci` · `deploy-from-registry-image` |
