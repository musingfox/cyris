...
| 35 | Reader-adjustable type size | Home decided 2026-09-20: one site-wide grade-D key, `digest.type_scale` (0.875, 1 or 1.125), set on `/settings` → Digest and required like every grade-D key (§5 row *Reader type size*). Saving it rescales `/settings` itself; no other page applies it yet | The app Worker reads the key from D1 and injects it into every HTML page it serves, so issues already published follow it too — not baked in at render time | `reader-type-size-setting` |
