...
| 34 | One design language beyond the tokens | #16 unified colour tokens only. Components still differ by surface — square px on the digest pages, 4–16px radii and rem on triage and `/settings` — no surface has a focus style, navigation lives in page footers, and `/settings` is one unsorted column whose only back link is the 404'd `/triage` | `docs/design/ui-language.md`, accepted 2026-09-17, with `docs/design/prototype.html` as its reference implementation: shared components as `_*.css.j2` partials copied into `style.css`, a site bar plus a Digest / All articles switch per issue, and `/settings` split into hash tabs (Model, Digest, Notifications, Sources). The spec's §8 is the landing order, five steps that each ship alone | `ui-spec-tokens-and-type` → `ui-site-bar-and-issue-bar` → `settings-page-layout` → `raw-page-triage-view` → `settings-value-origin-per-key` → `digest-index-archive-layout` → `digest-issue-page-layout`. The deck gets no restyle: it retires once raw has a triage view (`triage-raw-list-merge`) |
