...
| Discord webhook | D | **D1 `settings`**, written by `/settings`; `cyris.toml [notify]` for a `json` deployment | done — the URL is a posting token, and D1 stores it in plaintext. Anyone who can read `settings` can post to the channel; anyone who can open `/settings` can rotate it without a redeploy. That is the trade that makes it D. It is **not** among the seven counted below; those are API tokens, which stay C. `""` means off: `/settings` stores it only through a confirmed Turn off, and an empty paste is refused. No environment variable supplies it |
...
**Five numbered items are open — #9, #13, #14, #28 and #29 — plus the two unnumbered rows under
...
open:    one dashboard glance at Containers usage that M5's receipt still wants
...
| ~~35~~ | ~~Reader-adjustable type size~~ | Done 2026-09-20: one site-wide grade-D key, `digest.type_scale` (0.875, 1 or 1.125), set on `/settings` → Digest and required like every grade-D key (§5 row *Reader type size*). The app Worker reads it from D1 over REST and injects it into every HTML page it serves, so issues already published follow it too, and a save rescales `/settings` at once — see *Every HTML page the Worker serves carries the reader's type size* above. The digest masthead's stats column is the one column width that follows the scale, and only above 1, so the larger title clears the stats card from 721px up while scale 1 keeps its exact layout. There is no site-bar control, and pages.dev opened directly stays at 1 | — | `reader-type-size-setting` |
