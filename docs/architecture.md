...
| **D · Runtime-mutable** | **D1 `settings`** (a `json` deployment: `cyris.toml`) | a write, effective next run | the reader, in the UI |
...
has a home and a writer: the twenty keys in `GRADE_D_KEYS` each have a `/settings` field, D1 is the
