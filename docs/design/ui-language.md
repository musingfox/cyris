...
- **所有間距都用刻度。** digest 內文原本的 14/18/22/28/36/44/56px 等值在第 7 步（`digest-issue-page-layout`）
  搬到刻度上，之後沒有例外。
...
2026-09-17 盤點的差距中，第 1 到 3 步已於 2026-09-18 落地，第 4、6、7 步已於 2026-09-19 落地：第 1 步
...
頭版卡片加年月面板，同一天的後續列以淡化的日期區分；第 7 步依 §6 重做 digest 內文，每個 section 只由
標籤印一次名字，標題層級依結構決定，`.meta` 收成一條基底規則，兩個內文網格共用一條流體欄寬規則，只剩
720px 一個斷點，內文間距全部搬上刻度。其餘差距由下面第 5 步處理。
